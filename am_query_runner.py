"""AM-Query: bounded, execution-verified SQL self-training.

Fresh business-data research; Qwen-derived adapter, not a foundation model trained
from scratch. No customer data or paid model API is used by this experiment.
Run --help for the allocated-GPU command-line interface.
"""

# %% Configuration and local experiment records
import argparse
import contextlib
import dataclasses
...
# %% Synthetic business schemas and independent task partitions
FAMILIES = (
...
# %% Read-only SQL verification with resource limits
ALLOWED_FUNCTIONS = {
...
# %% GPU preflight and immutable model provenance
...
# %% QLoRA engine: one allocated GPU, assistant-token loss only
class Engine:
...
# %% Durable training checkpoints and interruption recovery
...
# %% Evaluation caches, paired selection gates, and honest uncertainty
...
# %% Model-generated examples: retain only execution-verified answers
...
# %% Experiment controller: baseline, supervised seed, then verified self-training
...
# %% One frozen final test: baseline, supervised-only, and selected champion
...
# %% Export and a synthetic demonstration; nothing is automatically published
...
# %% Command-line entry point for an already allocated GPU session
def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--profile",choices=["smoke","pilot"],default="smoke")
    parser.add_argument("--root",default=None)
    parser.add_argument("--hours",type=float,default=None)
    parser.add_argument("--action",choices=["train","final","export","status","check","demo"],default="train")
    args = parser.parse_args()
    cfg = Config.for_profile(args.profile,args.root,args.hours)
    existing = Path(cfg.root).expanduser()/"manifest.json"
    if existing.exists():
        # Reuse notebook-customized immutable settings when moving to batch execution.
        saved = read_json(existing)["config"]
        cfg = Config(**saved,root=cfg.root,session_hours=cfg.session_hours)
    if args.action == "check":
        data_self_check()
    elif args.action == "status":
        print(json.dumps(read_json(Path(cfg.root)/"state.json"),indent=2))
